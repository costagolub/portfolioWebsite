	...
	$('form#send-form').submit(function() {
			...
			$('.requiredField').each(function() {
				if($.trim($(this).val()) == '') {
					...
					$(this).parent().append('<span class="help-inline">Вы забыли указать '+labelText+'.</span>');
					$(this).parents('.control-group').addClass('error');
					hasError = true;
				} else if($.trim($(this).val()) == '' && $(this).hasClass('subject')){
						$(this).parent().append('<span class="help-inline">Вы не указали Тему письма.</span>');
						$(this).parents('.control-group').addClass('error');
						hasError = true;
					...
					if(!emailReg.test($.trim($(this).val()))) {
						var labelText = $(this).parent('.controls').prev('label').text();
						$(this).parent().append('<span class="help-inline">Вы указали неправильный email.</span>');
						$(this).parents('.control-group').addClass('error');
						hasError = true;
					}
					...
					if(spamValue !== '86') {
						var labelText = $(this).parent('.controls').prev('label').text();
						$(this).parent().append('<span class="help-inline">Вы указали неправильный '+labelText+'.</span>');
						$(this).parents('.control-group').addClass('error');
						hasError = true;
					}
				}
			...
			if(!hasError) {
				...
				$.post($(this).attr('action'),formInput, function(data){
					$('form#send-form').slideUp("fast", function() {				   
						$(this).before('<h2 class="text-center text-success">Спасибо! Ваше письмо отправлено. Я постараюсь не задерживаться с ответом!</h2>');
						setTimeout(function(){
		...
		function skillstimer(){
			...
			document.getElementById('years').innerHTML = years + 'х лет';
